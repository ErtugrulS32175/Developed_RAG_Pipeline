"""Handle-bound WRITE transport. PACKAGE B2B-C2.

A SECOND transport, and deliberately not an extension of the first.
`fs_transport` is the evidence walker: it declares a FORBIDDEN_ACCESS set
containing every write right and is asserted never to request one, which
is exactly the property that makes its answers evidence. Adding a write
path to it would retire that property for every caller it already has.
...
if _WINDOWS:                                   # pragma: no cover - platform
    from tools.agent_loop import application_transport_windows as _impl
else:                                          # pragma: no cover - platform
    from tools.agent_loop import application_transport_posix as _impl
