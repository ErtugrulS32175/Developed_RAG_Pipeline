...
OpenWebUI. Both engines answer the same contract, so `eval/rag_eval.py` measures
...
def _configure_models():
    ...
    from pipeline import embeddings, query
    ...
    Settings.llm = OpenAILike(
        model=query.LLM_MODEL_NAME,
        api_base=query.LLM_API_URL.rsplit("/v1/", 1)[0] + "/v1",
...
def answer(question):
    ...
    Reusing `query.build_context` and `query.generate` keeps the citation format
    and the grounding instructions identical across engines. Otherwise a
